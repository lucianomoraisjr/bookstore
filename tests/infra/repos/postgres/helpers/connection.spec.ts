...
describe('PgConnection', () => {
  ...
  it('should get repository from queryRunner', async () => {
    await sut.connect()
    await sut.openTransaction()
    const repository = sut.getRepository(PgBook)

    expect(getRepositorySpy).toHaveBeenCalledWith(PgBook)
  ...
  it('should return ConnectionNotFoundError on getRepository if connection is not found', async () => {
    ...
    expect(() => sut.getRepository(PgBook)).toThrow(new ConnectionNotFoundError())
  })
})
